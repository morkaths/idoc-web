          ...
          item.dropdown ? (
            <NavigationMenuItem key={item.label}>
              ...
              <NavigationMenuContent>
                ...
                >
                  {item.hero && (
                    <li className='row-span-4 m-0 p-0'>
                      <NavigationMenuLink asChild>
                        <Link
                          className='m-0 flex h-full w-full items-stretch justify-stretch rounded-md bg-transparent p-0'
                          href={item.hero.href}
                          tabIndex={-1}
                        >
                          {item.hero.image && (
              ...
              >
                <Link href={item.href || '#'} className='flex items-center gap-2'>
                  <div className='flex items-center gap-2'>
                    {item.icon &&
                      React.createElement(item.icon, { className: 'w-4 h-4 text-foreground' })}
                    {item.label}
                  </div>
                </Link>
              </NavigationMenuLink>
