    ...
    const renderSettingsContent = () => (
        <div className="space-y-4">
            <div className="space-y-2">
                <h4 className="font-medium leading-none text-muted-foreground flex items-center gap-2">
                    ...
                    {EPUB_THEMES.map((t) => (
                        <button
                            ...
                            className={cn(
                                ...
                                theme === t.value ? "border-primary shadow-sm" : "border-transparent opacity-80 hover:opacity-100"
                            )}
                            onClick={() => {
                                setTheme(t.value);
                                setIsOpen(false);
            ...
            <div className="space-y-2">
                ...
                <div className="grid grid-cols-2 gap-2">
                    {EPUB_FLOWS.map((f) => (
                        ...
                        >
                            {f.value === 'paginated' ? <BookOpen className="h-4 w-4" /> : <ScrollText className="h-4 w-4" />}
                            <span>{f.label}</span>
                        </Button>
                    ))}
                </div>
    ...
    if (isMobile) {
        return (
            <Sheet open={isOpen} onOpenChange={setIsOpen}>
                ...
                <SheetContent side="bottom" className="rounded-t-xl px-6 pb-10 pt-6" container={portalContainer}>
