...
export const BookApi = {
  find: async (
    params?: FindParams
  ): Promise<{ data: Book[]; pagination?: Pagination }> => {
...
export const BookApiMock = {
  find: async (
    ...
    if (filters && Object.keys(filters).length) {
      ...
      const categoryCondition =
        (filters as any).category ?? (filters as any).categories
      if (categoryCondition != null) {
        ...
        items = items.filter((item) => {
          ...
          const categoryIdsFromItem: string[] = Array.isArray(
            (item as any).categories
          )
            ? (item as any).categories.map((c: any) =>
                String(c._id ?? c.id ?? c)
              )
            : []
          ...
          return selectedIds.every((selectedId) =>
            categoryIdsFromItem.includes(selectedId)
          )
        })
      }
    }
  ...
  update: async (id: string, data: Partial<Book>): Promise<Book | null> => {
    ...
    return new Promise((resolve) =>
      setTimeout(() => resolve(mockBooks[index]), 200)
    )
  },
  delete: async (id: string): Promise<boolean> => {
    const index = mockBooks.findIndex((book) => book._id === id)
