...
type NavigationItem = {
  ...
  href?: string;
} & ({ type: 'page'; children?: never } | { type: 'category'; children: NavigationItem[] });
...
export function NavbarMobile({ items, onNavigate }: NavbarMobileProps) {
  ...
  const NavigationMenuItemMobile = ({ item, level }: { item: NavigationItem; level: number }) => {
    if (item.type === 'page') {
      return (
        <Link href={item.href || '#'} className='block' onClick={onNavigate}>
          <div
            className='focus-visible:ring-ring/50 hover:bg-muted hover:text-primary flex items-center gap-2 rounded-md p-1 transition-colors outline-none focus-visible:ring-[3px]'
    ...
    return (
      ...
      >
        <CollapsibleTrigger className='focus-visible:ring-ring/50 hover:bg-muted hover:text-primary flex items-center gap-2 rounded-md p-1 transition-colors outline-none focus-visible:ring-[3px]'>
          {item.icon ? (
            <item.icon className='size-4 shrink-0' />
          ) : (
            <CircleSmallIcon className='size-4 shrink-0' />
