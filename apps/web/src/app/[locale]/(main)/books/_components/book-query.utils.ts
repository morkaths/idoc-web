...
import { commaSeparatedArray, numericParam, parseSearchParams, buildQueryString } from '@/utils/query';
...
export const DEFAULT_BOOK_SORT: BookSortState = { sortBy: DEFAULT_BOOK_SORT_FIELD, sortOrder: DEFAULT_BOOK_SORT_ORDER };

export const parseBookQuery = (params: URLSearchParams) => parseSearchParams(bookQuerySchema, params);
...
export const buildBookFindParams = (
  ...
  query: filter.query,
  filters: [
    ...(filter.categories.length ? [{ field: 'categories.id', value: filter.categories, operator: FilterOperator.IN }] : []),
    ...(filter.languages.length ? [{ field: 'language', value: filter.languages, operator: FilterOperator.IN }] : []),
  ],
