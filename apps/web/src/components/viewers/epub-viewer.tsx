 ...
 */
const applyRenditionStyles = (
  rendition: Rendition,
  theme: Theme,
  fontFamily: string,
  fontSize: number
) => {
  const activeTheme = EPUB_THEMES.find((t) => t.value === theme) ||
    EPUB_THEMES[0] || { fg: '#000', bg: '#fff' };
  const currentFont =
    fontFamily === 'Origin'
      ? 'inherit'
...
export function EpubViewer({ fileUrl, className }: EpubViewerProps) {
  const { settings, getEpubLocations, saveEpubLocations } = useReaderSettings();
  ...
  const handleLocationChange = useCallback((epubcfi: string | number) => {
    ...
    if (rendition?.book?.locations) {
      ...
      try {
        const percentage =
          typeof loc.percentageFromCfi === 'function'
            ? loc.percentageFromCfi(epubcfi as string)
            : 0;
        setProgress(Math.round(percentage * 100));

        const current =
          typeof loc.locationFromCfi === 'function' ? loc.locationFromCfi(epubcfi as string) : null;
  ...
  const getRendition = useCallback(
    (rendition: Rendition) => {
      ...
      rendition.hooks.content.register((contents: Contents) => {
        const iframe = (contents as unknown as { window?: { frameElement?: Element } }).window
          ?.frameElement;
        if (iframe) {
    ...
    [cacheKey, getEpubLocations, saveEpubLocations, handleLocationChange, settings]
  ); // settings deps ở đây ổn vì locationsInitializedRef sẽ chặn việc nạp lại
  ...
  return (
    <div
      ref={containerRef}
      className={`relative flex h-full w-full flex-col overflow-hidden rounded-md border transition-colors ${className || ''}`}
      style={
        {
    ...
    >
      ...
      <div className='pointer-events-none absolute top-3 right-3 left-3 z-10 flex items-center justify-between'>
        <div className='pointer-events-auto'>
          <Sheet open={isTocOpen} onOpenChange={setIsTocOpen}>
            ...
            <SheetContent
              side='left'
              className='flex w-[300px] flex-col border-r p-0 transition-colors sm:w-[400px]'
              style={{ backgroundColor: themeConfig.tocBg, borderColor: themeConfig.uiBorder }}
              container={containerRef.current}
            >
              <SheetHeader className='border-b p-4' style={{ borderColor: themeConfig.uiBorder }}>
                ...
                <div className='space-y-1 p-2'>
                  {toc.map((item, index) => (
                    <button
                      ...
                      className='w-full truncate rounded-md px-4 py-2 text-left text-sm transition-colors'
                      style={{
        ...
        <div className='pointer-events-auto flex items-center gap-2'>
          ...
          <EpubSettings themeConfig={themeConfig} portalContainer={containerRef.current} />
        </div>
      </div>
      ...
      {/* Progress Bar & Status */}
      <div
        className='z-10 flex h-10 items-center justify-between border-t px-6 transition-colors'
      ...
      >
        <div className='flex flex-1 items-center gap-4'>
          <div className='bg-muted/30 relative h-1.5 flex-1 overflow-hidden rounded-full'>
            <div
              className={`absolute inset-y-0 left-0 rounded-full transition-all duration-300 ease-out ${isCalculating ? 'animate-pulse opacity-50' : ''}`}
              style={{
        ...
        <div
          className='ml-4 flex items-center gap-3 text-[11px] font-bold tracking-widest uppercase opacity-80'
