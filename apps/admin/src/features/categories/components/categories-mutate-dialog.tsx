...
type CategoriesMutateDialogProps = {
  ...
  onSubmit: (data: CategoryRequest & { id?: string }) => void;
};
            ...
            <fieldset disabled={isSubmitting} className='space-y-4'>
              ...
              <div className='max-h-72 space-y-3 overflow-auto'>
                {fields.map((field, idx) => {
                  const selectedLangs = fields
                    .map((f, i) => (i !== idx ? f.lang : null))
                    .filter(Boolean);

                  return (
                    <div
                      key={field.id}
                      className='bg-muted/40 relative space-y-2 rounded border p-4'
                    >
                      {/* Nút xoá bản dịch */}
                      {fields.length > 1 && (
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <div className='grid gap-3'>
                            <FormLabel htmlFor={`translations.${idx}.description`}>
                              Description
                            </FormLabel>
                            <FormControl>
