...
export function BookListItems({
    ...
    return (
      <div className={`flex flex-col gap-3 sm:gap-4 ${className}`}>
        {[...Array(5)].map((_, i) => (
          <div
            key={i}
            className="flex w-full bg-white dark:bg-zinc-900 rounded-lg sm:rounded-xl overflow-hidden border border-gray-100 dark:border-zinc-800"
          >
            {/* Left Accent Area w/ Cover Skeleton */}
            <div className="w-24 sm:w-40 shrink-0 bg-primary/10 dark:bg-primary/20 flex items-center justify-center p-1 sm:p-3">
              <Skeleton className="w-20 sm:w-28 aspect-[3/4] rounded-sm" />
            </div>

            {/* Content Skeleton */}
            <div className="flex-1 p-2 sm:p-6 flex flex-col justify-center min-w-0">
              <Skeleton className="h-2 sm:h-3 w-10 sm:w-12 mb-2 sm:mb-3" /> {/* Tag */}

              <Skeleton className="h-4 sm:h-7 w-full sm:w-3/4 mb-2 sm:mb-3" /> {/* Title */}

              <Skeleton className="h-3 sm:h-4 w-1/2 sm:w-1/3 mb-2 sm:mb-4" /> {/* Author */}
              ...
              {/* Bottom Row */}
              <div className="mt-auto pt-1.5 sm:pt-3 border-t border-gray-50 dark:border-zinc-800/50 flex items-center justify-between">
  ...
  return (
    <BookmarkProvider>
      <div className={`grid grid-cols-1 xl:grid-cols-2 gap-3 sm:gap-4 xl:gap-6 ${className}`}>
        {data.map((book) => (
          <BookListItem
            key={book.id}
            book={book}
