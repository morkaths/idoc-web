    ...
    <div className='mx-auto min-h-screen w-full max-w-5xl lg:border-x'>
      <div className='mx-4 grid h-[calc(100vh-3.5rem)] grid-cols-1 border-x md:mx-0 md:grid-cols-2 md:border-x-0'>
        <div className='space-y-4 px-4 pt-12 pb-4 md:border-r'>
          <h2 className='text-2xl font-bold tracking-tight md:text-3xl lg:text-4xl lg:font-extrabold'>
            FAQs
          </h2>
          <p className='text-muted-foreground'>
            Here are some common questions and answers that you might encounter when using iDoc.
          </p>
        </div>
        <div className='place-content-center'>
          <Accordion collapsible defaultValue='item-1' type='single'>
            {questions.map((item) => (
              <AccordionItem
                className='data-[state=open]:bg-card first:border-t last:border-b'
                key={item.id}
                value={item.id}
              >
