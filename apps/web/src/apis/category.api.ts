...
export const CategoryApi = {
  find: async (
    params?: FindParams
  ): Promise<{ data: CategoryResponse[]; pagination?: Pagination }> => {
    const response = await ApiClient.get<CategoryResponse[]>(API_CONFIG.endpoints.category.find, {
      mode: 'public',
      params,
    });
    return {
  ...
  create: async (data: CategoryRequest): Promise<CategoryResponse> => {
    const response = await ApiClient.post<CategoryResponse>(API_CONFIG.endpoints.category.create, {
      mode: 'private',
      data,
    });
    if (response.success && response.data) return response.data;
  ...
  delete: async (id: string): Promise<boolean> => {
    const response = await ApiClient.delete<null>(API_CONFIG.endpoints.category.delete(id), {
      mode: 'private',
    });
