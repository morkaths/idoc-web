...
export const BorrowApi = {
  ...apiFactory<BorrowResponse, BorrowRequest>(ApiEndpoint.endpoints.borrows, 'Borrow', {
    find: 'public',
    findById: 'public',
  }),
  ...
  history: async (
    ...
    const response = await ApiClient.get<BorrowResponse[]>(ApiEndpoint.endpoints.borrows.history, {
      security: 'private',
      params,
    });
    return {
  ...
  extend: async (id: string, extraDays: number, note?: string): Promise<BorrowResponse> => {
    const response = await ApiClient.put<BorrowResponse>(ApiEndpoint.endpoints.borrows.extend(id), {
      security: 'private',
      data: { extraDays, note },
    });
  ...
  return: async (id: string): Promise<BorrowResponse> => {
    const response = await ApiClient.put<BorrowResponse>(ApiEndpoint.endpoints.borrows.return(id), {
      security: 'private',
    });
    if (response.success && response.data) return response.data;
